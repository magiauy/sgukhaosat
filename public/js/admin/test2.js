...
import { callApi } from '/js/apiService.js';

// Constants
const API_BASE_URL = '/api/v2/statistics'; // Full API path with /api prefix
 ...
 * Using the project's standard callApi function instead of direct fetch
...
async function fetchAPI(endpoint, method = "GET", data = null) {
    ...
    try {
        // Add a cache-busting parameter to the URL for GET requests
        if (method === "GET") {
            const cacheBuster = new Date().getTime();
            endpoint += (endpoint.includes('?') ? '&' : '?') + '_=' + cacheBuster;
        }
        
        console.log(`[DEBUG] Sending request to: ${endpoint}`);
        
        // Use the project's callApi function which handles tokens and errors
        const response = await callApi(endpoint, method, data);
        console.log(`[DEBUG] Response received:`, response);
        
        // Return the response directly since callApi already handles errors and parsing
        return {
            status: true,
            data: response
        };
...
async function loadDashboardStats() {
    ...
    // Use the updated fetchAPI with proper API path
    const data = await fetchAPI(`${API_BASE_URL}/dashboard`, "GET");
...
async function loadFormList() {
    ...
    const data = await fetchAPI(`${API_BASE_URL}/forms/responses`, "GET");
...
async function loadFormDetails(formId) {
    ...
    const data = await fetchAPI(`${API_BASE_URL}/forms/${formId}`, "GET");
...
async function loadFormResponseTrend(formId) {
    ...
    const data = await fetchAPI(`${API_BASE_URL}/forms/${formId}/responses/trend?timeframe=${timeframe}&days=${days}`, "GET");
...
async function loadQuestionStatistics(formId) {
    ...
    const data = await fetchAPI(`${API_BASE_URL}/forms/${formId}/questions`, "GET");
    if (data.status) {
...
async function loadTimeAnalysis() {
    ...
    const data = await fetchAPI(`${API_BASE_URL}/time-analysis`, "GET");
    if (data.status) {
...
function checkAuthStatus() {
    ...
    // Using callApi which automatically handles authentication issues
    fetchAPI(`${API_BASE_URL}/dashboard`, "HEAD")
        .then(response => {
            if (!response.status) {
                console.error('[DEBUG] Not authenticated, redirecting to login');
                window.location.href = '/login'; // Use consistent login path with callApi
            } else {
